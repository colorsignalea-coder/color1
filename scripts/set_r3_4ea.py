# coding: utf-8
"""
set_r3_4ea.py — R3 EA 자동 선택 + Gen + Run [TEST 1콤보]
#   AITRP1 3fc → AI_TRIPLE_3_1_GoldBTC_0323_3fc
#   BRaid2 3fc → BRaid_SAR_3_2_GoldBTC_0323_3fc
#   TRange 3fc → BTC_Gold_TrendRange_EA_0323_3fc
#   LRC0 3fc → LRC_EA_3_0_GoldBTC_0323_3fc
사용법:
  python set_r3_4ea.py          # 현재 R3 EA만 선택 후 Gen+Run
  python set_r3_4ea.py all       # 모든 EA 체크 ON (Gen+Run 안 함)
  python set_r3_4ea.py none      # 모든 EA 체크 OFF
"""
import sys, time
...
ALL_EA_LABELS = set(['AITRP1 3fc', 'AI_TRIPLE', 'AI_TRIPLE 2fc', 'BRaid2 3fc', 'BRaid_SAR', 'BRaid_SAR 2fc', 'B_DFT2', 'B_FB5', 'COLOR175', 'COLORBB B', 'COLORBB G', 'FB5_2Sma', 'LRC0 3fc', 'LRC_EA', 'LRC_EA 2fc', 'SupplyDem', 'SupplyDemand 2fc', 'Survivor 2fc', 'Survivor v2', 'TRange 3fc', 'TrendRange', 'TrendRange 2fc', 'euraka'])
R_EA_KEEP     = set(['AITRP1 3fc'])
...
for hwnd,text in ea_h:
    ...
    set_check(hwnd,want)
    print(f"  {'[R3] ' if want else '       '}{text}: {'ON ✅' if want else 'off'}")
...
print(f"\nDONE: R3 백테스트 시작! (1EA x 1SL x 1TP = 1콤보 [TEST])")
