...
; =====================================================
; 4단계 백테스트 자동화 NC (No-Keyboard/Mouse) nc2.3
; base: SIMPLE_4STEPS_NC22.ahk (nc2.2) → nc2.3 개선판
; - nc2.3: --since 중복분석 방지, ROOT 폴더 분석
...
if (terminalPath != "NONE" && terminalPath != "") {
    ...
    if (eaPath != "NONE" && eaPath != "" && filenameEA != "" && filenameEA != "NONE") {
...

        {
            ...
            ; [v8.0 fix] SOLO 날짜 설정을 tester.ini에 직접 동기화 (GUI만 바꾸면 MT4가 무시함)
...
; [v8.0 fix] 날짜 읽은 후 tester.ini에 재동기화
if (testFromDate != "" && testToDate != "") {
    IfExist, %testerIniPath%
    {
        IniWrite, %testFromDate%, %testerIniPath%, Tester, FromDate
...
; 덮어쓰기/확인 팝업 처리 (10회, 200ms 간격 = 최대 2초)
Loop, 10 {
    IfWinExist, ahk_class #32770
    {
        WinGetText, dlgText, ahk_class #32770
        ; 덮어쓰기 팝업 - 텍스트 무관하게 #32770 창이 남아있으면 Enter로 확인
        if (dlgText != "") {
            FileAppend, Overwrite/confirm dialog detected (text=%dlgText%)`n, %logFile%
            Send, {Enter}
            Sleep, 300
            break
        }
        ; 텍스트 없어도 혹시 남아있으면 Enter
        Send, {Enter}
        Sleep, 300
        break
    }
