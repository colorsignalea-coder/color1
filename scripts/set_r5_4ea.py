# coding: utf-8
"""
set_r5_4ea.py — R5 EA 자동 선택 + Gen + Run [TEST 1콤보]
...
사용법:
  python set_r5_4ea.py          # 현재 R5 EA만 선택 후 Gen+Run
  python set_r5_4ea.py all       # 모든 EA 체크 ON (Gen+Run 안 함)
  python set_r5_4ea.py none      # 모든 EA 체크 OFF
"""
import sys, time
...
for hwnd,text in ea_h:
    ...
    set_check(hwnd,want)
    print(f"  {'[R5] ' if want else '       '}{text}: {'ON ✅' if want else 'off'}")
...
print(f"\nDONE: R5 백테스트 시작! (1EA x 1SL x 1TP = 1콤보 [TEST])")
