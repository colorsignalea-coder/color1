# coding: utf-8
"""
set_r6_8ea.py — R6 EA 자동 선택 + Gen + Run
#   AITRP1 6fc → AI_TRIPLE_3_1_GoldBTC_0323_6fc
#   BRaid2 6fc → BRaid_SAR_3_2_GoldBTC_0323_6fc
#   TRange 6fc → BTC_Gold_TrendRange_EA_0323_6fc
#   LRC0 6fc → LRC_EA_3_0_GoldBTC_0323_6fc
"""
import sys, time
...
ALL_EA_LABELS = set(['AITRP1 3fc', 'AITRP1 4fc', 'AITRP1 5fc', 'AITRP1 6fc', 'AI_TRIPLE', 'AI_TRIPLE 2fc', 'BRaid2 3fc', 'BRaid2 4fc', 'BRaid2 5fc', 'BRaid2 6fc', 'BRaid_SAR', 'BRaid_SAR 2fc', 'B_DFT2', 'B_FB5', 'COLOR175', 'COLORBB B', 'COLORBB G', 'FB5_2Sma', 'LRC0 3fc', 'LRC0 4fc', 'LRC0 5fc', 'LRC0 6fc', 'LRC_EA', 'LRC_EA 2fc', 'SupplyDem', 'SupplyDemand 2fc', 'Survivor 2fc', 'Survivor v2', 'TRange 3fc', 'TRange 4fc', 'TRange 5fc', 'TRange 6fc', 'TrendRange', 'TrendRange 2fc', 'euraka'])
R_EA_KEEP     = set(['AITRP1 6fc', 'BRaid2 6fc', 'LRC0 6fc', 'TRange 6fc'])
...
print(f"\n[EA] R6 8개 EA 선택 (1R=off 2R=off 3R+=선택):")
for hwnd,text in ea_h:
    want = text in R_EA_KEEP
    set_check(hwnd,want)
    tag = "[R6] " if want else "      "
    print(f"  {tag}{text}: {'ON ✅' if want else 'off'}")

for hwnd,text in lot_h: set_check(hwnd, text=="0.1")
for hwnd,text in sl_h:  set_check(hwnd, text in {"300","500"})
for hwnd,text in tp_h:  set_check(hwnd, text in {"300","500"})
...
print(f"\nDONE: R6 8EA 백테스트 시작! (8EA x 2SL x 2TP = 32콤보)")
