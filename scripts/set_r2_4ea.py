# coding: utf-8
"""set_r2_4ea.py — R2 EA 자동 선택 + Gen + Run [TEST 1콤보]
#   AITRP1 2fc → AI_TRIPLE_3_1_GoldBTC_0323_2fc
#   BRaid2 2fc → BRaid_SAR_3_2_GoldBTC_0323_2fc
#   TRange 2fc → BTC_Gold_TrendRange_EA_0323_2fc
#   LRC0 2fc → LRC_EA_3_0_GoldBTC_0323_2fc"""
import sys, time
...
def set_check(hwnd, want):
    ...
    if cur!=tgt:
        ...
        win32api.PostMessage(p,WM_COMMAND,(BN_CLICKED<<16)|(cid&0xFFFF),hwnd); time.sleep(0.15)

def click(hwnd):
    p=win32gui.GetParent(hwnd); cid=win32gui.GetDlgCtrlID(hwnd)
    win32api.PostMessage(p,WM_COMMAND,(BN_CLICKED<<16)|(cid&0xFFFF),hwnd); time.sleep(0.2)
...
ALL_EA_LABELS = set(['AITRP1 2fc', 'AI_TRIPLE', 'AI_TRIPLE 2fc', 'BRaid2 2fc', 'BRaid_SAR', 'BRaid_SAR 2fc', 'B_DFT2', 'B_FB5', 'COLOR175', 'COLORBB B', 'COLORBB G', 'FB5_2Sma', 'LRC0 2fc', 'LRC_EA', 'LRC_EA 2fc', 'SupplyDem', 'SupplyDemand 2fc', 'Survivor 2fc', 'Survivor v2', 'TRange 2fc', 'TrendRange', 'TrendRange 2fc', 'euraka'])
R_EA_KEEP     = set(['AITRP1 2fc'])
...
if not gen_hwnd or not run_hwnd:
    print("버튼 전체:"); [print(f"  {h}: '{t}'") for h,t in buttons]; sys.exit(1)

if MODE=="all":
    [set_check(h,True) or print(f"  ✅ {t}: ON") for h,t in ea_h]; sys.exit(0)
if MODE=="none":
    [set_check(h,False) or print(f"  ⬜ {t}: off") for h,t in ea_h]; sys.exit(0)

if stop_hwnd: click(stop_hwnd); time.sleep(1)
for hwnd,text in ea_h:
    want = text in R_EA_KEEP
    set_check(hwnd,want)
    print(f"  {'[R2] ' if want else '       '}{text}: {'ON ✅' if want else 'off'}")
...
print(f"\nDONE: R2 시작! (1EA x 1SL x 1TP = 1콤보 [TEST])")
