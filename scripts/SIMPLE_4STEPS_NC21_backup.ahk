...
; =====================================================
; 4단계 백테스트 자동화 NC (No-Keyboard/Mouse) nc2.1
; base: SIMPLE_4STEPS_NC.ahk (nc2.0) → nc2.1 개선판
; - nc2.1: Report탭 중복 WinActivate 제거 → WinActivate 10회 (원본 22회 대비 55% 절감)
...
logFile := A_WorkingDir "\scripts\simple_4steps_nc21_log.txt"
...
if (terminalPath != "NONE" && terminalPath != "") {
    ...
    if (eaPath != "NONE" && eaPath != "" && filenameEA != "" && filenameEA != "NONE") {
...

        {
            ...
            FileAppend, [TESTER.INI] Updated: Expert=%eaRelativePath% Symbol=%targetSymbol% Period=%testerPeriod%`n, %logFile%
