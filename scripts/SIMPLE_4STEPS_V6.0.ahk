...
SetMouseDelay, 50
SetControlDelay, -1  ; [NC2.0] ControlClick 딜레이 제거 (NA 옵션과 함께 사용)

; =====================================================
; 4단계 백테스트 자동화 NC (No-Keyboard/Mouse) v5.3
; base: SIMPLE_4STEPS_NC22.ahk (nc2.2) → v5.3 개선판
; - v5.3: --since 중복분석 방지, ROOT 폴더 분석
; - NC: WinActivate 제거, ControlClick NA + ControlSend 사용
; - 레포트 저장(우클릭+s+Ctrl+V+Enter)만 키보드/마우스 사용 (필수)
...
logFile := A_WorkingDir "\scripts\simple_4steps_nc23_log.txt"
...
if (terminalPath != "NONE" && terminalPath != "") {
    ...
    if (eaPath != "NONE" && eaPath != "" && filenameEA != "" && filenameEA != "NONE") {
...

        {
            ...
            ; [v5.3] 모델링 모드 강제 고정: 0=모든틱, 1=제어점, 2=개방가격만
            IniWrite, 0, %testerIniPath%, Tester, Model

            ; [v8.0 fix] SOLO 날짜 설정을 tester.ini에 직접 동기화
            ...
            FileAppend, [TESTER.INI] Updated: Expert=%eaRelativePath% Symbol=%targetSymbol% Period=%testerPeriod% Model=0 From=%testFromDate% To=%testToDate%`n, %logFile%
...
Sleep, 100
; [NC2.0 FIX] 1회 WinActivate - MT4가 tester.ini 재로드 + EA 목록 갱신 필수
; (이 1회 없으면 EA ComboBox가 빈 상태로 이전 EA 재실행됨)
...
; =====================================================
; [NC2.0] DETECT_STEP2_COORDS F6 캘리브레이션 좌표 읽기 (EA 드롭다운 정밀 클릭)
; =====================================================
IniRead, _eaDDRelX, %iniFile%, coords, relxSysTrading, 0
IniRead, _eaDDRelY, %iniFile%, coords, relySysTrading, 0
_eaDDRelX += 0.0
_eaDDRelY += 0.0
if (_eaDDRelX > 0 && _eaDDRelY > 0) {
    _eaDDAbsX := wx + Round(_eaDDRelX * ww)
    _eaDDAbsY := wy + Round(_eaDDRelY * wh)
    FileAppend, [EA-DD] Calibrated coords: relx=%_eaDDRelX% rely=%_eaDDRelY% abs=(%_eaDDAbsX%,%_eaDDAbsY%)`n, %logFile%
} else {
    _eaDDAbsX := 0
    _eaDDAbsY := 0
    FileAppend, [EA-DD] No calibrated coords - will use ControlClick fallback`n, %logFile%
}
...
if (filenameEA != "" && filenameEA != "NONE") {
    ...
    FileAppend, [v4.8 EARLY] EA typing: %earlySearchEA%`n, %logFile%
    ; [NC2.0 FIX] MT4 ComboBox 커스텀 컨트롤 → WinActivate + 캘리브레이션 클릭 + Send (1회)
    ; MT4 ControlGet,Choice는 커스텀 컨트롤에서 항상 빈값 → 재시도 불가, 1회만 실행
    WinActivate, ahk_id %mt4ID%
    WinWaitActive, ahk_id %mt4ID%,, 3
    Sleep, 600
    if (_eaDDAbsX > 0 && _eaDDAbsY > 0)
        Click, %_eaDDAbsX%, %_eaDDAbsY%        ; F6 캘리브레이션 좌표 (정밀)
    else
        ControlClick, %eaComboClassNN%, ahk_id %mt4ID%  ; fallback
    Sleep, 400
    Send, {Home}
    Sleep, 300
    Send, %earlySearchEA%
    ...
    Send, {Enter}
    Sleep, 500
    FileAppend, [v4.8 EARLY] Clicked & typed: %earlySearchEA%`n, %logFile%
}
...
; [NC2.0 FIX] STEP 0-A: v4.8 EARLY 결과 확인 → 불일치 시 재시도
Sleep, 300

if (filenameEA != "" && filenameEA != "NONE") {
    searchEA := StrReplace(filenameEA, ".ex4", "")
    ; EARLY에서 이미 1회 클릭함 → STEP 0-A는 tester.ini 확인 로그만
    FileAppend, [v3.7] EA set via EARLY: %searchEA% (tester.ini also updated)`n, %logFile%
} else {
...
if (testDateEnable = 1) {
    ...
    if (fromDateEditNN != "ERROR" && fromDateEditNN != "" && testFromDate != "") {
        ...
        ; [NC2.0] ControlSend (날짜 입력 - 포커스 불필요)
        ControlClick, x%clickX% y%clickY%, ahk_id %mt4ID%,,,, NA
        Sleep, 100
        ControlSend, %fromDateEditNN%, %fromYear%{Right}%fromMonth%{Right}%fromDay%{Enter}, ahk_id %mt4ID%
        Sleep, 100
    ...
    if (toDateEditNN != "ERROR" && toDateEditNN != "" && testToDate != "") {
        ...
        ControlClick, x%clickX% y%clickY%, ahk_id %mt4ID%,,,, NA
        Sleep, 100
        ControlSend, %toDateEditNN%, %toYear%{Right}%toMonth%{Right}%toDay%{Enter}, ahk_id %mt4ID%
        Sleep, 100
...
; =====================================================
; [v5.3] STEP 0-E: SET 파일 로드 -> tester.ini 직접 수정 방식
; =====================================================
...
if (setFilePath != "ERROR" && setFilePath != "" && FileExist(setFilePath)) {
    ...
    if (lotsValue != "") {
        ...
        ; [NC2.0] ControlSend 방식 (WinActivate 불필요)
        ControlSend,, ^{Tab}, ahk_id %mt4ID%
        Sleep, 200
...
if (termPathG != "NOTSET") {
    ...
    if (setParamFile != "NOTSET" && setParamFile != "") {
        ...
        if (FileExist(comboSetPath)) {
            ...
            ; [NC2.0] WinActivate removed
            Sleep, 200
            ControlSend,, {F7}, ahk_id %mt4ID%  ; [NC2.0]
            ...
            if (epHwnd != 0) {
                ...
                if (WinExist("ahk_id " . epHwnd)) {
                    ...
                    if (!okClicked) {
                        ControlSend,, {Enter}, ahk_id %epHwnd%
...
ControlClick, %startButtonClassNN%, ahk_id %mt4ID%,,,, NA
Sleep, 200
...
if (!InStr(btnText, "중지") and !InStr(btnText, "Stop")) {
    ...
    ; [NC2.0] ControlClick NA (Start 버튼 fallback)
    ControlClick, x%absX% y%absY%, ahk_id %mt4ID%,,,, NA
}
...
Loop {
    ...
    isStopButton := (InStr(buttonText, "중지") or InStr(buttonText, "Stop") or InStr(buttonText, "||") or InStr(buttonText, "| |"))
    ...
    if (progressPct >= 100 and !isStopButton) {
    ...
    if (isStopButton) {
        buttonWasStop := true
    } else if (buttonWasStop and !isStopButton) {
    ...
    if (elapsed > 150000 and !isStopButton) {
        FileAppend, Complete: Timeout + Start button`n, %logFile%
        break
    }

    if (elapsed > 600000) {
        FileAppend, Complete: Hard Timeout 10min`n, %logFile%
        break
    }
...
; MT4 윈도우 강제 활성화 (리포트 탭 저장용 - 원본 방식 유지)
WinActivate, ahk_id %mt4ID%
Sleep, 200
...
; [nc2.1] 중복 WinActivate 제거 (L1436에서 이미 활성화됨)
